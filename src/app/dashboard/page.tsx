...
interface Task {
  ...
  due_date: string;
  assigned_to: { id: string; name: string }[]; // Updated to array
  project_id: { id: string; name: string } | null;
}
...
export default function DashboardPage() {
  ...
  const [newTask, setNewTask] = useState({
    title: "",
    project_id: "",
    assigned_to: [] as string[], // Updated to array
    due_date: "",
  });
  ...
  const fetchData = useCallback(async (token: string) => {
    try {
      ...
      if (projectsRes.ok) {
        ...
        setProjects(pData);
        if (pData.length > 0) {
          setNewTask(prev => ({ ...prev, project_id: prev.project_id || pData[0].id }));
        }
      }
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!editingProject) {
          setNewTask(prev => ({ ...prev, project_id: data.id }));
        }
      } else {
        const data = await res.json();
    ...
    <div className="dashboard-layout">
      <aside className="sidebar">
        ...
        <nav className="project-list">
          <div 
            className={`project-item ${!selectedProjectId ? "project-item-active" : ""}`}
      ...
      <main className="main-content">
        <div className="stats-bar">
          ...
          <div className="stat-card">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <span className="stat-value" style={{ color: stats.overdue > 0 ? "#ef4444" : "inherit" }}>
                {stats.overdue}
              </span>
            </div>
          </div>
        ...
        <header className="section-header">
          <h1 style={{ fontSize: "1.875rem" }}>
            {selectedProjectId ? projects.find(p => p.id === selectedProjectId)?.name : "Dashboard"}
          </h1>
          {user?.role === "Admin" && (
        ...
        <div className="task-list">
          {tasks.map((task) => (
            <div key={task.id} className="task-card">
              ...
              >
                {task.status === "Done" ? (
                  <CheckCircle2 size={24} color="var(--accent)" fill="var(--accent-foreground)" />
                ) : (
              ...
              <div className="task-info">
                ...
                <div className="task-meta">
                  ...
                  <span style={{ opacity: 0.7, display: "flex", alignItems: "center", gap: "6px" }}>
                    <Folder size={14} /> {task.project_id?.name}
                  </span>
                </div>
              </div>
              {user?.role === "Admin" && (
        ...
        <div className="modal-overlay" onClick={() => { setShowModal(false); setEditingTask(null); }}>
          <div className="modal-content" style={{ maxWidth: "600px" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
              ...
              <button className="btn-icon" onClick={() => { setShowModal(false); setEditingTask(null); }}>
                <X size={20} />
              </button>
            </div>
